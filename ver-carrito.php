...
<body>
	...
	<div id="view-cart" class="form-group">
		...
		<h1 class="col-sm-12">
			<?php if(isset($_SESSION['usuario'])){?>
			<button id="buy-cart" class="btn btn-primary">Comprar</button>  
			<?php }else{ ?>
			<h3>Inicia sesión o regístrate para hacer el pedido</h3>
			<?php } ?>
			<button id="clear-cart" class="btn btn-primary">Limpiar Canasta</button>
		</h1>
		<?php }else{ ?>
